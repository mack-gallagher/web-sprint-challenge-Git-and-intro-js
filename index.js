...
console.log('Name of the first artist in the array: ',artists[0].name);
...
console.log('This should be Vincent Van Dough: ',artists[8]);

artists[8].name = 'Vincent Van Gogh';
console.log('9th artist [this should be Vincent Van Gogh, with all the correct information: ',artists[8]);
...
function artistInfo(arr, artistName){
  const index = arr.findIndex(x => x.name === artistName);
  return arr[index].bio;
}
...
function artistByCountry(arr, whichCountry){
  const fromCountry = [];
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].nationality === whichCountry) {
      fromCountry.push(arr[i].name);
    }
  }
  return fromCountry;
}
